...
var app = angular.module('jwype', []); // Initialise angular app in 'JSmart'

// Proxy
var chatHub = $.connection.chatHub; // set signalr hub proxy

/* Moment.js Implementation */
var moment = moment(); // initialise moment.js

var seff = new Audio('../sounds/incomingmsg.wav'); // Initialise sound effect for incoming message

/**********************************************************************************
                    ************** CHAT ****************
**********************************************************************************/
var totalOnlineUsers = 0, totalAvailableGroups = 0; // Available users online or groups

function emoticonFiltering(message) { // set up message emoticon
    var filtered = '';
    var smileys = [
...
function showAllGroupsList(chatHub, groupList) { //Group List in a sidebar
    var html = '';
...
function makeGroupChatWindow(chatHub, groupid, groupWindowId, groupname) { //Create a group window
                    ...
                    '<div class="panelChat panel-primary">' +
                        ...
                        '<div class="panel-footer">' +
                            '<div class="input-group">' +
                                ...
                                '</span>' +
                                '<span class="input-group-btn">' +
                                    '<button class="btn btn-danger btn-sm" id="btnJwing">Hello!!!</button>' +
                                '</span>' +
                            '</div>' +
                        '</div>' +
                    '</div>' +
    ...
    $('#group_' + groupid).find('#btnJwing').click(function () {
        chatHub.server.sendBroadcastMessage(groupname, $('#hidUsername').val(), '<font style="color:red">Hello!</font>', Date.now());
    });
...
function openGroupChatWindow(chatHub, id, groupname) { // open chat window group
...
function initEvents(chatHub) { // Initialise event listener for a button input
...
function registerUser(chatHub, id, name) { // Register user online to the online user list
...
function addMessage(groupid, groupname, username, message, timestamp, firstOpen) { // Create a bubble chat message window
...
function openPrivateChatWindow(chatHub, id, username) { // Create chat window user
...
function makePrivateChatWindow(chatHub, userid, windowid, username) { // 
    ...
    $('#' + windowid).find('#btnJwing').click(function () {
        chatHub.server.sendPrivateMessage(userid, '<font style="color:red">Hello!</font>', Date.now());
    });
...
function EnterAdminRoom() { // Admin Authentication
    ...
    if (password != '' && password == 'young5050') {
        $('#adminPwd').val('');
        $('#adminLoginModal').modal('hide');
        makeGroupChatWindow(chatHub, 1, 'group_1', 'Administrator');
    }
    else {
        alert("Incorrect Password Entered please try diffrent one");
...
function clientManager(chatHub) { // Callback Client SignalR Manager
    ...
    chatHub.client.onConnected = function (id, username, Users, Messages, groupList) { // When client is connected to the server
    ...
    chatHub.client.onNewUserConnected = function (id, name) { // When a user is disconnected from the server
        //display an info indicating a connected user is online
        totalOnlineUsers++;
        $('#totalOnlineUsers').html(totalOnlineUsers);
        registerUser(chatHub, id, name);

        var log_on = $('<div class="mainbox col-md-6 col-md-offset-3 col-sm-8 col-sm-offset-2" style="position:fixed; top:10px; right:0px">' +
                            ...
                            '</div>' +
                    '</div>');
        //append log on user to the chatWindow container
        $(log_on).hide();
        $('#chatWindowContainer').append(log_on);
        $(log_on).fadeIn(400).delay(5000).fadeOut(400);


    }

    chatHub.client.onUserDisconnected = function (id, username) { // When a user is disconnected from the server
        $('#' + id).remove();
        ...
        $(log_off).fadeIn(300).delay(3000).fadeOut(300);
    }


    chatHub.client.messageReceived = function (groupid, groupname, username, message, timestamp) { //On recieving of message from client
        addMessage(groupid, groupname, username, message, timestamp, false);
        seff.play();
    }

    chatHub.client.sendPrivateMessage = function (windowid, senderName, message, timestamp) { // Send a private message
        ...
        var bubbleChat = '<li class=\'left clearfix\'>' +
                                ...
                                '<div class=\'chatBox-body clearfix\'>' +
                                    '<div class=\'header\'>' +
                                        '<legend class=\'primary-font\'>' + senderName + '</legend>' +
                                        '<small class=\'pull-right text-muted\'>' +
                                            '<span class=\'glyphicon glyphicon-time\'></span>' +
                                            moment.zone("+06:00").utc(timestamp, 'X').format('YYYY-MM-DD HH:mm') +
                                        '</small>' +
                                    '</div>' +
                                    '<p>' + emoticonFiltering(message) + '</p>' +
                                '</div>' +
...
/**********************************************************************************
                    ************** VIDEO CONFRENCE & THE REST PART ****************
**********************************************************************************/
function enterRoomConf(roomId) { // Create a Conference Room
...
onBistriConferenceReady = function () {
    ...
    // Initialise Available Room List
    var html = '';
    ...
    window.localStream; //Video streaming
    // Joining a room
    window.joinRoom = function (roomId) {
        if (!window.localStream) {
            alert('Local media is not ready.');
            console.log("Can't access webcame at this moment \nSorry please try again later ." + "\nType of error\t")
            //return;
        }
    ...
    bc.signaling.bind("onJoinedRoom", function (result) { // A user should request to join a room
    ...
    bc.signaling.bind("onJoinRoomError", function (error) { //Display a ROOM JOIN ERROR
        console.log('Error: ' + error.text + ' (' + error.code + ')');
    });

    bc.signaling.bind("onQuittedRoom", function (data) { // Signal if a client leaves a room

    });

    bc.streams.bind("onStreamAdded", function (remoteStream, pid) { // Attach stream to the container
        var node = document.querySelector('.video-container');
    ...
    bc.streams.bind("onStreamClosed", function (remoteStream, pid) { //  (leave room)
        bc.detachStream(remoteStream);
    });

    // enter the AppId and AppKey created with my accounts
    bc.init({
        appId: '88abfb72',
